    ...
    let activeFolderId = null; // 記錄目前選取的資料夾
    ...
    function toggleSidebarMenu() {
      ...
      if (isMobile) {
        ...
        // Desktop collapse toggle — the grid column collapses on body, sidebar width follows
    ...
    function renderBreadcrumb() {
      ...
      // Expands every ancestor folder (and the world) of a target folder in the sidebar tree,
      // mirroring how clicking a Windows Explorer breadcrumb segment reveals that path in the nav pane.
      ...
      // Builds a dropdown listing a folder's/world's immediate children (subfolders first, then docs),
      // like the chevron dropdown on a Windows Explorer address-bar segment.
      ...
      // 1. Worldview Breadcrumb Item — click navigates to the world; hover shows its top-level folders/docs
      ...
      // 2. Folder Hierarchy Items — click navigates/selects + expands the tree to reveal it; hover shows its children
      const folderChain = [];
    ...
    /* ==========================================================
       4. INLINE RENAME (MOBILE DOUBLE-TAP / DESKTOP RIGHT-CLICK & DBLCLICK)
       ========================================================== */
    ...
    /* ==========================================================
       5. TREE RENDERING WITH FOLDER DRAG & FOLDER CREATE
       ========================================================== */
    function renderSidebarTree() {
      ...
      appData.worldviews.forEach(function(world) {
        ...
        const isWorldCollapsed = !!collapsedFolders[world.id];
        const worldDiv = document.createElement("div");
        worldDiv.className = "world-block " + (isCurrent ? "active-world" : "");

        const header = document.createElement("div");
        header.className = "world-header";
        header.onclick = function(e) {
          if (e.target.closest('.world-caret') || e.target.closest('.node-icon') || e.target.closest('.icon-btn-group')) return;
          activeWorldId = world.id;
          updateWorldBadge();
          renderSidebarTree();
        ...
        const worldHasKids = worldHasChildren(world.id);
        const worldCaretGlyph = !worldHasKids ? '' : (isWorldCollapsed ? '▸' : '▾');

        header.innerHTML = 
          '<div class="world-title-wrap">' +
            '<span class="world-caret" style="cursor:' + (worldHasKids ? 'pointer' : 'default') + ';">' + worldCaretGlyph + '</span>' +
            '<span class="node-icon" onclick="event.stopPropagation(); openIconPicker(\'world\', \'' + world.id + '\')">' + (world.icon || '🌐') + '</span>' +
            '<span class="node-name">' + escapeHtml(world.name) + '</span>' +
          '</div>' +
          '<div class="icon-btn-group">' +
            '<button class="icon-btn" title="在此外層新增資料夾" onclick="event.stopPropagation(); promptCreateFolder(null, \'' + world.id + '\')">📁＋</button>' +
            '<button class="icon-btn" title="在此外層新增文檔" onclick="event.stopPropagation(); createNewDoc(null, \'' + world.id + '\')">📄＋</button>' +
          '</div>';

        if (worldHasKids) {
          header.querySelector('.world-caret').onclick = function(e) {
            e.stopPropagation();
            collapsedFolders[world.id] = !collapsedFolders[world.id];
            renderSidebarTree();
          };
        }

        setupRenameTriggers(header.querySelector('.node-name'), 'world', world.id, function() { return world.name; });

        worldDiv.appendChild(header);

        const bodyDiv = document.createElement("div");
        bodyDiv.className = "world-body";
        if (isWorldCollapsed && !search) bodyDiv.style.display = "none";

        renderFolderLevel(world.id, null, bodyDiv, search);
        worldDiv.appendChild(bodyDiv);
    ...
    function worldHasChildren(worldId) {
      return appData.folders.some(function(f) { return f.worldId === worldId && f.parentId === null; }) ||
             appData.docs.some(function(d) { return d.worldId === worldId && !d.folderId; });
    }
    ...
    function renderFolderLevel(worldId, parentId, parentElement, search) {
      ...
      folders.forEach(function(folder) {
        ...
        // Windows Explorer only draws an expand arrow when there's something to expand
        ...
        folderRow.ondrop = function(e) {
          ...
          try {
            ...
            if (dragPayload.type === "doc") {
              ...
              // Move folder inside this folder (prevent moving into itself or its descendants)
    ...
    function createDocRowElement(doc) {
      ...
      row.onclick = function() {
        ...
        activeFolderId = null; // 點擊文檔時，自動清除資料夾的選取狀態
        updateWorldBadge();
    ...
    // Direct create inside targeted folder
