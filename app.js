  ...
  folders.forEach(function(folder) {
    ...
    folderRow.innerHTML = 
      ...
      '<div class="node-actions">' +
        '<button class="icon-sm-btn" title="在此資料夾建檔" onclick="event.stopPropagation(); createNewDoc(\'' + folder.id + '\', \'' + worldId + '\')">📄＋</button>' +
        '<button class="icon-sm-btn" title="新增子資料夾" onclick="event.stopPropagation(); promptCreateFolder(\'' + folder.id + '\', \'' + worldId + '\')">📁＋</button>' +
        '<button class="icon-sm-btn" title="移動" onclick="event.stopPropagation(); promptMoveFolder(\'' + folder.id + '\')">🔀</button>' +
      '</div>';
...
function createDocRowElement(doc) {
  ...
  let displayTitle = doc.title || "無標題文檔";
...
function renderBreadcrumb() {
  ...
  const world = appData.worldviews.find(function(w) { return w.id === doc.worldId; }) || { name: "主世界觀", icon: "🌐" };

  const worldItem = document.createElement("span");
  worldItem.className = "breadcrumb-item";
  worldItem.textContent = (world.icon || '🌐') + " " + world.name;
  ...
  folderChain.forEach(function(folder) {
    ...
    folderItem.textContent = (folder.icon || '📁') + " " + folder.name;
...
function renderCanvasLines() {
  ...
  canvas.edges.forEach(function(edge) {
    ...
    path.onclick = function() {
      ...
      if (val === null) return;
      if (val.trim() === "") canvas.edges = canvas.edges.filter(function(e) { return e.id !== edge.id; });
...
function openBatchExportModal() {
  ...
  appData.worldviews.forEach(function(w) {
    ...
    const docs = appData.docs.filter(function(d) { return d.worldId === w.id; });
...
function toggleExportAll(status) {
  document.querySelectorAll(".export-checkbox").forEach(function(cb) { cb.checked = status; });
}
...
function confirmBatchExport() {
  ...
  const ids = Array.from(checkedBoxes).map(function(cb) { return cb.getAttribute("data-id"); });
  const format = document.getElementById("exportFormatSelect").value;
  const docs = appData.docs.filter(function(d) { return ids.includes(d.id); });
...
function openPaletteModal() {
  ...
  Object.keys(DEFAULT_PALETTES).forEach(function(key) {
    ...
    row.innerHTML = 
      ...
      '<input type="text" class="form-input" id="pal_name_' + key + '" value="' + escapeHtml(pal.name) + '">';
    container.appendChild(row);
  });
...
function closePaletteModal() {
  Object.keys(DEFAULT_PALETTES).forEach(function(key) {
    ...
    if (input && input.value.trim()) appData.colorPalette[key].name = input.value.trim();
  });
  saveData();
...
function openColorPicker(tag, anchorElement) {
  ...
  popover.innerHTML = '<div style="font-size:11px; font-weight:700; color:var(--text-muted); margin-bottom:4px;">指定顏色：</div>';

  Object.keys(DEFAULT_PALETTES).forEach(function(key) {
    const pal = appData.colorPalette[key] || DEFAULT_PALETTES[key];
    const opt = document.createElement("div");
    opt.className = "picker-option";
    opt.innerHTML = `<span style="width:12px; height:12px; border-radius:3px; background:${pal.bg}; border:1px solid ${pal.text};"></span><span style="color:${pal.text}; font-weight:600;">${escapeHtml(pal.name)}</span>`;
